...
- **Status:** completed
- **Ticket:** API-005
- **Goal:** Add the first reports module behind the existing `/v1/reports/summary` and `/v1/reports/breakdown` contract, with deterministic period totals, grouped rollups, and top-N metadata, without widening into exports, staging-only closeout, or mobile sync integration.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `README.md` only if repo-truth scope changes, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `docs/04_API_Contract.yaml` if response/query metadata changes, `server/api/src/app.ts`, `server/api/src/modules/module.ts` only if needed for wiring, `server/api/src/modules/reports/*`, and focused API tests.
- **Rationale:** The re-audit on 2026-03-29 still leaves `SET-002` blocked because `gh` is not installed and no `GH_TOKEN` or `GITHUB_TOKEN` is available. `CAP-003` and `CAP-004` remain blocked because `adb` cannot currently start a usable daemon from this environment. `UX-005`, `INT-005`, `API-003`, and `API-004` are still active in backlog tracking, but their remaining gaps are external acceptance, mobile integration, or staging evidence rather than missing repo-side implementation. `API-005` is now the earliest actionable canonical `todo`.
- **Risks:** Reports work can sprawl into exports, worker rollups, domain rewrites, or full analytics infrastructure. This pass must stay inside authenticated summary and breakdown read endpoints, deterministic aggregation over the existing durable domain store, contract alignment, and correctness-focused tests.
- **API / schema impact:** Expect an additive reports module under the existing OpenAPI paths. If the response shape needs top-N metadata beyond the current contract, update `docs/04_API_Contract.yaml` and run `pnpm lint:openapi`.
- **Rollout / flag plan:** No rollout flag. Keep the reports API additive and read-only on top of the current domain store.
...
- **Done when:** Summary and breakdown routes return deterministic period aggregates over the current domain data, the contract and route tests match that behavior, repo checks stay green, and the backlog ticket advances truthfully without being overstated as latency-certified or staged.
- **Outcome:** Added a new authenticated reports module with `GET /v1/reports/summary` and `GET /v1/reports/breakdown`, wired through the existing session layer and durable domain store. The summary route now returns canonical period totals, prior-period deltas, top merchant/category/item, and budget status snapshots; the breakdown route supports `category`, `merchant`, `item`, `hourOfDay`, and `dayOfWeek` plus top-N metadata (`limit`, `totalGroups`, `hasMore`). I extracted the budget-summary helper into `server/api/src/modules/domain/budget-summary.ts` so reports and domain budget detail share the same budget math. Added focused service and route tests for known-fixture correctness, auth, and query validation, updated the OpenAPI schema for the new breakdown metadata, and passed `pnpm --filter @upi-spend-tracker/api typecheck`, `pnpm --filter @upi-spend-tracker/api test -- --runInBand src/modules/reports`, `pnpm lint:openapi`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` on 2026-03-29. `API-005` remains `in_progress` in the canonical backlog because the ticket's done-when still requires latency-target evidence that is not defined or directly provable in this environment.
