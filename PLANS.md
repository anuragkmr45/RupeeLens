...
- **Status:** completed
- **Ticket:** API-004
- **Goal:** Continue the server-side domain API suite by adding the missing merchant and transaction-item CRUD/list surface so the ticket covers the remaining core domain entities named in the backlog and engineering model.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `README.md` only if repo-truth scope changes, `docs/04_API_Contract.yaml`, and `server/api/src/modules/domain/*` plus API runtime tests only if route wiring changes.
- **Rationale:** The re-audit on 2026-03-28 still leaves `SET-002` blocked because `gh` is not installed and no `GH_TOKEN` or `GITHUB_TOKEN` is available, and `CAP-003`/`CAP-004` remain blocked because `adb devices` is empty. `UX-005`, `INT-005`, and `API-003` also remain active but not closeable from this environment. `API-004` is still the earliest active canonical ticket, and the current repo truth shows the first domain slice covers transactions, categories, rules, and budgets but still lacks merchant and transaction-item endpoints even though the backlog and engineering model treat them as primary entities.
- **Risks:** API-004 is broad and can sprawl into reports, sync, alias-management, or full staging closeout claims. This pass must stay inside merchant and transaction-item CRUD/list behavior, optimistic concurrency, soft-delete behavior, field-specific validation, and contract alignment. It must not widen into reports (`API-005`), worker jobs (`API-007`), mobile sync client work, or staging-only acceptance claims.
...
- **Done when:** Merchant and transaction-item endpoints exist with contract-aligned validation, optimistic concurrency, and soft-delete/list semantics; repo checks stay green; and the backlog ticket advances truthfully without being overstated as staged or fully closed.
- **Outcome:** Continued `API-004` by adding the missing merchant lifecycle and transaction-item API surface on the server: merchants now support list/create/get/patch/delete with duplicate-label conflict handling, transaction items now support list/get/create/patch/delete with parent-transaction status recalculation, and domain list routes now support explicit `includeDeleted` queries so soft-deleted entities can be requested intentionally. Updated the OpenAPI contract with merchant and item schemas/routes plus deleted-record fields, expanded route-level integration coverage for merchant/item lifecycles and includeDeleted behavior, and passed `pnpm --filter @upi-spend-tracker/api typecheck`, `pnpm --filter @upi-spend-tracker/api test`, `pnpm lint:openapi`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` on 2026-03-28. The backlog ticket remains `in_progress` because write-version enforcement is still incomplete on the older delete routes and the done-when still requires staging availability.
