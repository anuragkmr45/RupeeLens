...
- **Status:** completed
- **Ticket:** CAP-004
- **Goal:** Re-run the blocked `CAP-004` connected-Android closeout path on the newly reattached device and truthfully either move the ticket to `done` or refresh its blocker note with current evidence.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, and repo-truth docs only if the ticket status changes. No product code changes are planned unless the connected test path exposes a narrow CAP-004 defect that must be fixed to complete this ticket.
- **Rationale:** The 2026-04-01 re-audit changed materially again: `adb devices` now shows the physical Android target `e342703` while `gh` is still unavailable and release tickets remain dependency-blocked. Per the workflow, blocked tickets that can potentially close now must be preferred before later work, and `CAP-004` is the earliest such ticket after the already-closed `CAP-003`.
- **Risks:** The device may still reject APK installation or surface a real repository/runtime defect. Keep the pass scoped to `CAP-004`; do not widen into `CAP-005` even if the device remains available after this closeout attempt.
- **API / schema impact:** None expected.
- **Rollout / flag plan:** No rollout change. This is a closeout-oriented Android validation pass over the existing Room-backed native capture repository.
- **Validation commands:** `adb devices`, `cd client/android && ./gradlew :app:testDebugUnitTest`, `cd client/android && ./gradlew :app:connectedDebugAndroidTest`, plus repo-wide `pnpm db:validate`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` if the closeout result changes ticket status.
...
- **Outcome:** Re-ran the `CAP-004` closeout path on 2026-04-01 after `adb devices` again showed the physical Android target `e342703`. `./gradlew :app:testDebugUnitTest` passed, but `./gradlew :app:connectedDebugAndroidTest` again failed before any instrumentation ran because the device canceled installation of `app-debug.apk` on `M2102J20SI - 13` with `INSTALL_FAILED_USER_RESTRICTED: Install canceled by user`. No product code changes were made. The canonical backlog ticket remains `blocked`, and its existing blocker note already matches this current evidence, so no backlog row rewrite was needed.
...
- **Status:** completed
- **Ticket:** QA-002
- **Goal:** Harden the current Android-first product around financial-behavior data by moving sync tokens into secure storage, enforcing safe transport rules, tightening privacy defaults and retention behavior, and documenting the resulting data-handling posture without widening into staging-only signoff or Play Console work.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, client sync persistence/session/runtime files, `client/src/features/export/local-export.ts`, native capture settings/repository files only where privacy defaults needed adjustment, focused client tests, and repo-truth/privacy docs (`README.md`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `docs/13_Privacy_Security_Baseline.md`, `client/src/lib/app-info.ts`).
- **Rationale:** The re-audit on 2026-03-30 still leaves `SET-002` blocked because `gh` is unavailable, and `CAP-003` plus `CAP-004` remain blocked because `adb devices` is empty. `QA-001` is still not actionable because it depends on blocked `SET-002`. `REL-001` and `REL-002` remain dependency-blocked. The other active tickets (`CAP-005`, `UX-005`, `UX-007`, `INT-005`, `INT-008`, `API-004`, `API-005`, `API-007`, `QA-003`, `QA-004`, `QA-005`, and `SYNC-001`) now only have external closeout gaps. `QA-002` is therefore the earliest canonical `todo` with a concrete repo-side implementation path because repo truth already includes the relevant API/session flow, settings surface, native retention primitives, destructive-edit audit events, and redacted exports, but still lacks secure token storage, transport enforcement, privacy-mode defaulting, and consolidated privacy-review documentation.
- **Risks:** This can sprawl into full compliance/legal work, platform account setup, or generic security theater. Keep the scope inside repo-side baseline controls named in the engineering design: secure token storage, HTTPS/TLS-only production transport, privacy-mode default behavior, raw notification retention policy wiring, redaction guarantees, and documentation/tests. Do not claim beta signoff, Play review approval, or staging rollout evidence.
- **API / schema impact:** No OpenAPI change is expected unless transport or auth error semantics change. Client persistence will likely shift token material out of SQLite into secure storage. Any local schema change must remain additive and migration-safe.
- **Rollout / flag plan:** Keep local-only mode intact and preserve developer-local HTTP flows only where explicitly justified for local development. Production-facing paths should reject insecure transport. If repo-side implementation lands cleanly but security signoff is still external, leave the canonical backlog ticket `in_progress`.
- **Validation commands:** Focused client sync/security tests, native tests only if capture retention/privacy wiring changes, `pnpm db:validate` if local persistence schema or migration files change, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check`. Run `pnpm lint:openapi` only if contract semantics change.
- **Done when:** Tokens are no longer stored in plain local SQLite settings, insecure production transport is rejected, privacy-mode defaults and retention/redaction behavior match the documented baseline, tests/docs are updated, and tracking truthfully reflects any remaining external signoff gap.
- **Outcome:** Completed the repo-side QA-002 hardening pass on 2026-03-30 by moving sync access/refresh tokens and expiry out of SQLite and into Expo SecureStore with legacy-key cleanup, enforcing HTTPS/TLS for sync/session/telemetry outside approved local development hosts, defaulting privacy mode to enabled in both the client persistence layer and native capture settings, aligning local backup metadata with the secure default, and documenting the current data-handling / retention baseline plus Play Data Safety review inputs in `docs/13_Privacy_Security_Baseline.md`. Focused client privacy/security tests plus repo-wide `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check` all passed on 2026-03-30. The canonical backlog ticket remains `in_progress` because its done-when still requires beta security sign-off and final Play Data Safety review evidence outside this environment.

### QA-005 — Implement Analytics, Crash Reporting, And Operational Dashboards

- **Status:** completed
- **Ticket:** QA-005
- **Goal:** Add a privacy-safe observability path that can record the core mobile quality signals named in the backlog, expose a versioned telemetry schema and authenticated dashboard API, and evaluate alert thresholds without widening into paid providers, staging-only operations, or unrelated security work.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `docs/04_API_Contract.yaml`, `packages/contracts/src/*`, client telemetry/runtime files plus focused app hooks/tests, server API telemetry module files/tests, worker observability files/tests if needed for alert evaluation, and repo-truth docs (`README.md`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `client/src/lib/app-info.ts`) only if the resulting evidence changes ticket status or the default-next note.
- **Rationale:** The re-audit on 2026-03-30 still leaves `SET-002` blocked because `gh` is unavailable, and `CAP-003` plus `CAP-004` remain blocked because `adb devices` is empty. `CAP-005`, `UX-005`, `UX-007`, `INT-005`, `INT-008`, `API-004`, `API-005`, `API-007`, `QA-003`, `QA-004`, and `SYNC-001` now only have external closeout gaps. `QA-005` is therefore the earliest remaining canonical `todo` with satisfied dependencies and a concrete repo-side implementation path, and repo truth currently has no telemetry contracts, mobile event logging, crash-style error reporting, or operational dashboard surface at all.
- **Risks:** This can sprawl into vendor SDK setup, broad security hardening, staged operations work, or invasive analytics on sensitive content. Keep the scope inside privacy-safe event schemas, local-first client queuing/flush, authenticated telemetry ingestion, dashboard/alert aggregation, and tests/docs. Do not add paid providers or log sensitive raw values.
- **API / schema impact:** New telemetry contracts and API routes are expected, so `docs/04_API_Contract.yaml` and `pnpm lint:openapi` are required. Avoid mobile DB schema changes unless they are clearly justified; prefer additive local persistence that does not destabilize the current store.
- **Rollout / flag plan:** Keep telemetry payloads privacy-safe and additive. Do not claim field-operational observability or beta-ready alerting without direct evidence; if the repo-side implementation is complete but operational rollout is still missing, leave the canonical backlog ticket `in_progress`.
- **Validation commands:** Focused contract, client, API, and worker tests for telemetry/dashboard behavior, plus `pnpm lint:openapi`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check`. Run `pnpm db:validate` only if this pass changes persistence schema or migrations.
- **Done when:** The repo has versioned privacy-safe telemetry contracts, the client records and flushes the core events named in the backlog, the backend exposes an authenticated operational dashboard with alert-threshold evaluation, tests/docs are updated, and tracking reflects any remaining operational or beta-only evidence gap truthfully.
- **Outcome:** Completed the repo-side QA-005 implementation on 2026-03-30 by adding versioned privacy-safe telemetry contracts, local-first client event builders plus queued/authenticated flush and global runtime-error capture, authenticated telemetry ingest/dashboard API routes with deterministic alert thresholds, OpenAPI coverage, and repo-truth documentation. `pnpm --filter @upi-spend-tracker/contracts typecheck`, `pnpm --filter @upi-spend-tracker/api typecheck`, `pnpm --filter @upi-spend-tracker/client typecheck`, focused telemetry tests, `pnpm lint:openapi`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check` all passed on 2026-03-30. The canonical backlog ticket remains `in_progress` because its done-when still requires operational telemetry to be available before beta, which is external rollout evidence rather than a missing repo-side implementation.
...
- **Status:** completed
- **Ticket:** SYNC-001
- **Goal:** Close the remaining client-side sync gap by provisioning, persisting, and refreshing pairing/session credentials on-device, then wiring those credentials into the existing outbox/runtime path without widening into beta-only closeout claims, admin tooling, or a second sync protocol.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `client/src/features/sync/*`, `client/src/app/SpendTrackerApp.tsx`, focused client tests, and repo-truth docs (`README.md`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `client/src/lib/app-info.ts`) only if the resulting evidence changes ticket status or the default-next note.
- **Rationale:** The re-audit on 2026-03-30 still leaves `SET-002` blocked because `gh` is unavailable, and `CAP-003` plus `CAP-004` remain blocked because `adb devices` is empty. `API-003` is now done, while `CAP-005`, `UX-005`, `UX-007`, `INT-005`, `INT-008`, `API-004`, `API-005`, `API-007`, and `QA-004` now only have external closeout gaps. The mobile outbox/runtime foundation already exists, but the client still hardcodes `hasSyncCredentials = false`, never provisions or persists session/pairing credentials, and always passes `credentials: null` into the sync runtime, so `SYNC-001` remains the earliest active canonical ticket with a real repo-side implementation gap.
- **Risks:** This can sprawl into full auth/account UX, secure-store hardening, or broader sync conflict resolution work. Keep the scope inside client credential provisioning, persistence, refresh, pairing-code generation/consumption, and honest Settings/queue wiring over the existing session and sync APIs.
- **API / schema impact:** No OpenAPI or server-schema change expected. Reuse the existing session and pairing endpoints; if sync persistence keys need to store client credentials, keep that change additive and local.
- **Rollout / flag plan:** Respect the current `sync_later` mode only. Keep local-only mode fully functional, and do not claim beta-stable multi-device sync beyond what this environment can prove.
- **Validation commands:** `pnpm --filter @upi-spend-tracker/client typecheck`, focused client tests for sync provisioning/runtime/persistence, `pnpm db:validate`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check`.
- **Done when:** The client can create or join a sync session, persist/refresh credentials, feed them into the current sync runtime truthfully, and tracking reflects whether the ticket still has only external beta-stability gaps.
- **Outcome:** Closed the remaining repo-side SYNC-001 gap on 2026-03-30 by adding a dedicated client session/pairing helper over the existing guest-session APIs, persisting sync credentials in scoped `sync_settings` keys without clobbering queue state, refreshing access tokens before sync runs, and wiring Settings to create or join a sync session and generate pairing codes. The existing outbox/runtime path now uses real stored credentials instead of hardcoded `null`, while Home, Timeline, and Settings stay honest about local-only vs paired-sync state. `pnpm --filter @upi-spend-tracker/client typecheck`, focused client sync tests, `pnpm db:validate`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check` all passed on 2026-03-30. The canonical backlog ticket remains `in_progress` because the only remaining gap is external: multi-device sync still needs stable beta evidence beyond what this environment can prove.

### CAP-006 — Bridge Native Capture Events Into React Native Store Safely

- **Status:** completed
- **Ticket:** CAP-006
- **Goal:** Expose pending native capture events safely to React Native, import them idempotently into the local transaction store, and route actionable-notification deep links into classify or split flows without making JS the capture-system source of truth.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `client/android/app/src/main/java/com/upispendtracker/client/capture/*` bridge files only where needed, `client/src/features/android-capture/native-capture.ts`, `client/src/features/spend-tracker/*` only where import helpers or transaction persistence need narrow changes, `client/src/app/SpendTrackerApp.tsx`, focused client/native tests, and repo-truth docs (`README.md`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `client/src/lib/app-info.ts`) only if behavior changes.
- **Rationale:** The re-audit on 2026-03-30 still leaves `SET-002` blocked because `gh` is not installed and no `GH_TOKEN` or `GITHUB_TOKEN` is available. `CAP-003` and `CAP-004` remain blocked because `adb devices` is empty again, so no connected-Android closeout can finish. `CAP-005` now has the native actionable-notification foundation, but the app still cannot import pending native captures into JS local state or route notification actions into classify/split screens. `CAP-006` is therefore the earliest active canonical ticket with a real repo-side implementation gap.
- **Risks:** This work can sprawl into sync-client outbox behavior, schema-heavy transaction redesign, or CAP-005/CAP-007 Android validation that still depends on unavailable devices. Keep the scope inside safe native bridge methods, idempotent local import, notification-action routing, and resilient JS error handling. Do not claim connected-Android validation that cannot run.
- **API / schema impact:** No OpenAPI changes expected. Avoid local schema changes unless import reliability truly requires them; prefer deterministic imported transaction IDs and additive metadata reuse where possible.
- **Rollout / flag plan:** Reuse the existing native capture pipeline and current diagnostics/privacy settings. Import should be local-first, safe on app launch/foreground, and tolerant of stale native data without duplicating transactions.
- **Validation commands:** Focused client tests for native capture import and routing, focused Android unit tests only if bridge logic changes there, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check`. Run `pnpm db:validate` only if persistence schema or migrations change. Connected Android validation remains best-effort only if a device becomes available again.
- **Done when:** JS can fetch pending native capture events on launch, native bridge failures do not crash the app, imported transactions are idempotent, actionable-notification routes open the correct classify/split flow with safe prefill, and tracking reflects any remaining environment-only validation gap truthfully.
- **Outcome:** Completed the repo-side CAP-006 bridge on 2026-03-30 by exposing pending native capture records plus replies through the Android module, emitting capture-change events to React Native, importing native captures idempotently into the local transaction store with deterministic IDs, and routing `upispendtracker://capture-action` deep links into the existing classify and split flows with reply-based prefill. Focused client coverage now proves import-on-launch, idempotent import helpers, and action-route classify handoff; `./gradlew :app:testDebugUnitTest`, `pnpm --filter @upi-spend-tracker/client typecheck`, focused client tests, repo-wide `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check` all passed. The canonical backlog ticket can now move to `done`; connected-Android action closeout remains a separate environment-only gap on `CAP-005`, not this bridge ticket.
...
- **Status:** completed
- **Ticket:** QA-004
- **Goal:** Add repo-side Expo/EAS OTA channel configuration, a runtime-version policy that protects native compatibility, and an explicit rollout/rollback playbook without widening scope into analytics, staging approval, or store-release operations.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `README.md`, `docs/06_Release_Plan.md`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `client/app.json` or `client/app.config.*`, `client/eas.json`, `client/package.json` only if OTA/runtime dependencies or scripts are required, bootstrap runtime helpers/tests only if channel/runtime query behavior must reflect the new release config, and a new OTA playbook doc only if that is the clearest place to keep the rollback matrix.
- **Rationale:** The re-audit on 2026-03-29 still leaves `SET-002` blocked because `gh` is not installed and no `GH_TOKEN` or `GITHUB_TOKEN` is available. `CAP-003` and `CAP-004` remain blocked because `adb devices` is empty in the current environment. `UX-005`, `INT-005`, `API-003`, `API-004`, `API-005`, and `API-007` are still active in backlog tracking, but their remaining gaps are external QA/design acceptance, mobile integration evidence, staging evidence, or undefined latency targets rather than missing repo-side implementation. `QA-004` is now the earliest remaining canonical `todo` with satisfied dependencies because `SET-006` and `API-006` are both done.
- **Risks:** OTA work can sprawl into EAS account setup, store submissions, analytics, or broader release automation. This pass must stay inside repo-side OTA channel config, runtime-version policy, release scripts/docs, rollback guidance, and the minimum client/runtime wiring needed to align bootstrap queries with the configured release channel/runtime.
- **API / schema impact:** No OpenAPI or backend schema change expected. Keep the ticket limited to client release configuration and release-engineering documentation unless a small bootstrap-query alignment change is required on the client.
- **Rollout / flag plan:** Configure isolated `internal`, `beta`, and `production` OTA channels and document how rollback uses those channels plus remote kill switches from `SET-006`/`API-006`. Do not claim that remote EAS project/channel setup has already happened unless it is directly verifiable here.
- **Validation commands:** `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, plus focused client tests if bootstrap query/runtime wiring changes. Run `pnpm lint:openapi` only if the API contract changes.
- **Done when:** The repo has clear OTA channel configuration, a documented runtime-version policy that blocks native-incompatible OTA updates, a concrete rollback/checklist playbook, validation stays green, and backlog tracking moves beyond `todo` truthfully without claiming remote engineering approval or staging rollout evidence that is unavailable here.
- **Outcome:** Completed the repo-side QA-004 implementation on 2026-03-29 by replacing the static Expo config with `client/app.config.ts`, adding `client/eas.json` channel profiles for `internal`, `beta`, and `production`, wiring `expo-updates` runtime/channel metadata into the bootstrap-config query path, and documenting rollout/rollback behavior in `docs/12_OTA_Runtime_Rollback.md`. `pnpm --filter @upi-spend-tracker/client exec expo config --type public`, `pnpm --filter @upi-spend-tracker/client lint`, `pnpm --filter @upi-spend-tracker/client typecheck`, `pnpm --filter @upi-spend-tracker/client test -- --runInBand __tests__/bootstrap-config.test.ts`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check` all passed. The canonical backlog ticket remains `in_progress` because real Expo project/update URL wiring, non-production OTA/rollback dry-run evidence, and engineering approval are external closeout requirements that were not available in this environment.

### API-007 — Implement Background Jobs For Rollups, Cleanup, And Export Generation

- **Status:** completed
- **Ticket:** API-007
- **Goal:** Add the first real worker job suite for rollups, cleanup, and export generation with a simple durable queue/store foundation, without widening into staging-only closeout, mobile sync work, or broader admin/export UI flows.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `README.md` only if repo-truth scope changes, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, worker runtime/config files under `server/worker/src/**`, and API-side storage helpers only if a worker job needs cleanup access to existing durable stores.
- **Rationale:** The re-audit on 2026-03-29 still leaves `SET-002` blocked because `gh` is not installed and no `GH_TOKEN` or `GITHUB_TOKEN` is available. `CAP-003` and `CAP-004` remain blocked because `adb devices` is empty in the current environment. `UX-005`, `INT-005`, `API-003`, `API-004`, and `API-005` are still active in backlog tracking, but their remaining gaps are external acceptance, mobile integration, staging evidence, or undefined latency targets rather than missing repo-side implementation. `API-007` is now the earliest actionable canonical `todo`.
- **Risks:** Worker job work can sprawl into full export APIs, staging infrastructure, or cross-process queue systems. This pass must stay inside durable worker-side rollups, cleanup, and export-job processing with delayed retries, plus the minimal storage hooks needed to operate on the current file-backed stores.
- **API / schema impact:** No OpenAPI change expected unless export-job request/response behavior changes. Keep contract changes out of scope unless they are required by the chosen worker slice.
- **Rollout / flag plan:** No rollout flag. Keep new jobs additive and safe if the corresponding stores are empty.
- **Validation commands:** `pnpm --filter @upi-spend-tracker/worker typecheck`, `pnpm --filter @upi-spend-tracker/worker test`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build`, plus `pnpm lint:openapi` only if the API contract changes.
- **Done when:** The worker runs more than heartbeat alone, the new jobs are deterministic and retry-safe on the current durable stores, repo checks stay green, and the backlog ticket advances truthfully without claiming staging operations are complete.
- **Outcome:** Added the first real worker-job suite on 2026-03-29 without widening scope into staging-only closeout or new API routes. The worker now reads the durable API file stores, refreshes daily/monthly report rollups into its own rollup store, prunes expired access tokens / revoked refresh tokens / stale pairing codes / expired export artifacts, and processes queued CSV export jobs with delayed retries plus dead-letter visibility. New runtime config paths and intervals were added, focused worker module tests now cover rollup generation, cleanup pruning, and export success/failure retry behavior, and `pnpm --filter @upi-spend-tracker/worker typecheck`, `pnpm --filter @upi-spend-tracker/worker test`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` passed. The canonical backlog ticket remains `in_progress` because its done-when still requires the worker runtime to be operational in staging.
