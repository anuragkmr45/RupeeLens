...
- **Status:** completed
- **Ticket:** SYNC-001
- **Goal:** Add the first mobile sync foundation by persisting a local outbox, tracking entity versions and conflicts, scheduling retry-safe sync attempts, and exposing honest UI hooks for sync status without widening into pairing UX, staging-only closeout, or unrelated backend work.
- **Touched files/modules:** `PLANS.md`, `docs/05_Backlog.md`, `docs/05_Backlog.csv`, `docs/05_Backlog.json`, `client/src/features/spend-tracker/persistence.ts`, `client/src/features/spend-tracker/db/migrations.ts`, `client/src/features/spend-tracker/db/migration-runner.ts` only if migration adoption needs tightening, a new client sync feature module under `client/src/features/sync/*`, `client/src/app/SpendTrackerApp.tsx`, focused client tests, and repo-truth docs (`README.md`, `docs/09_Project_Phase_Status.md`, `docs/10_Codex_Workflow.md`, `client/src/lib/app-info.ts`) only if behavior changes.
- **Rationale:** The re-audit on 2026-03-30 confirmed `SET-002` remains blocked because `gh` is unavailable in this environment, and `CAP-003` plus `CAP-004` remain blocked because `adb devices` is empty again. `CAP-005`, `UX-005`, `INT-005`, `INT-008`, `API-003`, `API-004`, `API-005`, `API-007`, and `QA-004` all still have external closeout gaps rather than missing repo-side implementation. The client still has no outbox, retry scheduler, network observation, sync queue persistence, or conflict hooks even though the backend sync API (`API-003`) and native capture import dependency (`CAP-006`) are now available, so `SYNC-001` is the earliest actionable canonical ticket.
- **Risks:** This can sprawl into pairing/auth UI, full remote-apply conflict resolution, secure token storage hardening, or broad persistence refactors. Keep the scope inside local-first outbox persistence, retry/backoff policy, chunked sync orchestration, honest blocked-until-paired behavior, and lightweight UI hooks for sync/conflict state. Do not claim beta-stable multi-device sync or paired-device end-to-end validation that this environment cannot prove.
- **API / schema impact:** No OpenAPI change expected. Additive local SQLite sync tables and client-side sync state are expected. Reuse the existing `API-003` contract rather than inventing a second mobile sync protocol.
- **Rollout / flag plan:** Respect the existing local-only vs sync-later preference. Queue writes only when sync mode is not `local_only`, keep the app fully usable offline, and surface clearly when sync is waiting on pairing/session credentials instead of pretending cloud sync is active.
- **Validation commands:** Focused client tests for outbox diffing, retry scheduling, persistence resume, and sync UI hooks; `pnpm db:validate`; `pnpm lint`; `pnpm typecheck`; `pnpm test`; `pnpm build`; and `git diff --check`. Run `pnpm lint:openapi` only if the contract changes.
- **Done when:** Local mutations enqueue sync operations unless local-only mode is selected, retry scheduling uses exponential backoff with jitter, queued/conflict state survives restart, the app exposes honest sync/conflict hooks in the current UI, and tracking reflects any remaining pairing/beta-only closeout gap truthfully.
- **Outcome:** Completed the repo-side SYNC-001 implementation on 2026-03-30 by adding a new mobile sync foundation under `client/src/features/sync/*`, additive SQLite tables for sync settings/entity versions/outbox/conflicts, deterministic outbox diffing from the existing local persisted state, exponential-backoff retry scheduling plus reachability-aware chunked sync runtime helpers, and honest sync/conflict queue cards in Home, Timeline, and Settings. Focused client coverage now covers outbox diffing, retry/conflict behavior, sync persistence resume, migration adoption, and app-level sync queue rendering; `pnpm --filter @upi-spend-tracker/client typecheck`, focused client tests, `pnpm db:validate`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, and `git diff --check` passed on 2026-03-30. The canonical backlog ticket remains `in_progress` because pairing/session credential provisioning and multi-device beta-stability evidence still sit outside what this environment can truthfully prove.
