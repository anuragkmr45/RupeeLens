...
| Domain APIs and reports | Service and route tests in `server/api/src/modules/domain` and `server/api/src/modules/reports` | OpenAPI lint plus route integration coverage | No | No | Staging and latency evidence still pending |
