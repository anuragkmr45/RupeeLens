...
| ------------------------ | -------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- |
| Android native baseline  | Android 13 or newer                    | Physical device with notification-listener support | `pnpm qa:smoke`, connected Android manual capture/action checklist, `:app:connectedDebugAndroidTest`                                                                                                                                                                                                   | Physical-device notification capture evidence still pending; local physical install was blocked by `INSTALL_FAILED_USER_RESTRICTED` on 2026-04-06 |
| Android compatibility    | Android 12 or newer                    | Emulator or secondary physical device              | Connected Android instrumentation plus smoke checklist                                                                                                                                                                                                                                                 | Android 15 / API 35 emulator smoke verified on 2026-04-06; local `assembleDebug`, `assembleRelease`, and `bundleRelease` also passed on 2026-04-06. Physical secondary-device coverage still pending |
...
| iOS shell-only           | iOS simulator current Expo SDK support | iPhone simulator                                   | `xcodebuild -workspace client/ios/UPISpendTracker.xcworkspace -scheme UPISpendTracker -configuration Debug -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.2' CODE_SIGNING_ALLOWED=NO build`, release-style simulator build, `pnpm --filter @upi-spend-tracker/client run build:ios`, plus shell smoke | Repo-side debug/release-style simulator builds plus shell smoke, dark mode, export share sheet, and relaunch persistence verified on `iPhone 17` and `iPhone 16e` on 2026-04-06 |
...
- Run local Android `assembleRelease` and `bundleRelease`, then confirm the merged release manifest stays free of debug-only permissions and dev-client URL schemes.
- Run connected Android instrumentation and manual capture/action checks on a physical notification-listener device before beta or store submission.
