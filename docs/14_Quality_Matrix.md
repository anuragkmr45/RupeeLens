...
| iOS shell-only           | iOS simulator current Expo SDK support | iPhone simulator                                   | `xcodebuild -workspace client/ios/UPISpendTracker.xcworkspace -scheme UPISpendTracker -configuration Debug -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 17,OS=26.2' CODE_SIGNING_ALLOWED=NO build`, `pnpm --filter @upi-spend-tracker/client run build:ios`, plus shell smoke | Repo-side simulator build and export verified on 2026-04-06; manual shell smoke still pending |
