...
- **Tracking:** Status: in_progress | Owner: codex | Started At: 2026-03-28 | Completed At: | Commit Ref: pending-local-commit | Tracking Notes: Continued `API-004` on 2026-03-28 by adding the missing merchant lifecycle and transaction-item API surface on the server: merchants now support list/create/get/patch/delete with duplicate-label conflict handling, transaction items now support list/get/create/patch/delete with parent-transaction status recalculation, and domain list routes now support explicit `includeDeleted` queries so soft-deleted entities can be requested intentionally. Updated `docs/04_API_Contract.yaml` with merchant and item routes/schemas plus deleted-record fields, expanded route-level integration coverage for merchant/item lifecycles and includeDeleted behavior, and passed `pnpm --filter @upi-spend-tracker/api typecheck`, `pnpm --filter @upi-spend-tracker/api test`, `pnpm lint:openapi`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, and `pnpm build` on 2026-03-28. Status remains `in_progress` because write-version enforcement is still incomplete on the older delete routes and the done-when still requires the domain API suite to be available in staging.
