...
- Home now includes quick-action entrypoints for budgets, history search, and local category management; Inbox supports local status, merchant, source-app, amount, and age filters plus skip/delete actions on a FlatList path validated against a seeded 1,000-item local queue; quick classify runs in a bottom sheet with explicit suggestion chips and a save-as-rule intent toggle; split items now opens a full-screen local editor with running totals, remainder handling, and partial-save visibility in Inbox; Timeline adds grouped local history, search/filter chips, transaction detail with parser context plus saved notes and classification/audit history, and local categories can be created, edited, deleted, merged, and reused across classify/split/manual flows. Budget creation and rule screens still arrive in later tickets
