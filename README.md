...
- Backend skeleton now boots API and worker runtimes through explicit module factories, tested env loaders, and service/repository seams; `GET /health`, a cacheable signed `GET /v1/bootstrap/config` endpoint with runtime-version compatibility checks, guest-session/device-pairing APIs, first authenticated sync push/pull APIs, and the first authenticated domain API slice for transactions/categories/rules/budgets are live, with durable on-disk session/pairing/sync/domain persistence under the API runtime’s local data path, but the broader domain suite is still incomplete and there are still no mobile-integrated sync flows yet
