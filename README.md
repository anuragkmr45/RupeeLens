...
- Android-first onboarding now includes notification education, source-app selection, budget-cycle choice, sync preference, and a cycle-aware local dashboard with budget progress, top items, recent activity, a filtered Inbox, quick classify flow, skipped-item recovery, and manual spend entry
...
- Home now includes quick-action entrypoints for budgets and search, while Inbox supports local status, merchant, source-app, amount, and age filters plus skip/delete actions; the actual budget creation, search, split, and rule screens still arrive in later tickets
