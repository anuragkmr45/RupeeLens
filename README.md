...
- Backend skeleton now boots API and worker runtimes through explicit module factories, tested env loaders, and service/repository seams; `GET /health`, `GET /v1/bootstrap/config`, and first guest-session/device-pairing APIs are live, but session state is still in-memory and resets on API restart, and there are still no domain APIs or sync implementation yet
