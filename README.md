...
- Backend skeleton now boots API and worker runtimes through explicit module factories, tested env loaders, and service/repository seams; `GET /health`, `GET /v1/bootstrap/config`, and first guest-session/device-pairing APIs are live, with durable on-disk session/pairing persistence under the API runtime’s local data path, but there are still no domain APIs or sync implementation yet
