...
- Android dev builds now include a native notification-listener service, per-app allowlist sync, raw snapshot persistence, a dedicated capture processor, auditable parser-registry output with package-specific and generic fallback parsers, and dedupe counters/threshold diagnostics in the Android bridge/UI; repo-side dedupe is implemented, but connected Android closeout is currently blocked because the attached device canceled debug APK installation, while the capture repository and spend-domain import are still pending
...
- No deployment CI, analytics providers, or full native capture repository/domain-import pipeline yet
