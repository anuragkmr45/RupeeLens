...
- Home now includes quick-action entrypoints for budgets and history search, while Inbox supports local status, merchant, source-app, amount, and age filters plus skip/delete actions on a FlatList path validated against a seeded 1,000-item local queue; quick classify runs in a bottom sheet with explicit suggestion chips and a save-as-rule intent toggle; split items now opens a full-screen local editor with running totals, remainder handling, and partial-save visibility in Inbox; and Timeline now adds grouped local history, search/filter chips, transaction detail with parser context plus saved notes and classification/audit history, and clear edit/delete handoff. Budget creation and rule screens still arrive in later tickets
