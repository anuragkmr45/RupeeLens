...
- Android dev builds now include a native notification-listener service, per-app allowlist sync, raw snapshot persistence, a dedicated capture processor, and a diagnostics card that reflects real listener permission state; parser, dedupe, and spend-domain import are still pending, and final listener closeout still needs an Android target that allows androidTest install and direct UI access
