...
Use `pnpm qa:smoke` for a faster cross-stack release-blocker pass, `pnpm qa:performance` for the repo-side performance budget documented in `docs/15_Performance_Budget.md`, and `pnpm qa:regression` for the full repo-side baseline documented in `docs/14_Quality_Matrix.md`.
...
- iOS: manual/local-first app shell development and UI verification only

The iOS shell is supported for development, but iOS payment notification capture and parity with Android native flows remain out of scope for v1. The shared shell now hides Android-only capture diagnostics and frames iPhone truthfully as manual/local-only in v1.
...
- `docs/15_Performance_Budget.md`: repo-side performance budgets, named performance suite, bundle-size gate, and remaining device/network evidence checklist
...
- The shared shell now uses safe-area-aware layout, adaptive light/dark appearance, and a persistent bottom primary navigation surface so iPhone builds no longer present Android-style capture copy or repeated top-tab chrome
- Android dev builds now include a native notification-listener service, per-app allowlist sync, a Room-backed native capture repository with legacy migration, raw snapshot persistence, auditable parser-registry output with package-specific and generic fallback parsers, connected-device-validated dedupe and replay protection, app-owned actionable review notifications with direct reply plus classify / split / skip persistence, a dedicated in-app diagnostics screen with supported parser inventory plus recent parse failures/logs, redacted debug-bundle sharing, dedupe counters/threshold diagnostics in the Android bridge/UI, and a safe native-to-JS capture bridge that imports pending native captures into local transactions with deterministic IDs and routes classify / split action deep links into the existing review flows. The remaining Android closeout work is now the Room-backed native capture repository closeout and connected-device manual validation for the notification-action path
...
- The repo-side `QA-003` performance pass now avoids off-screen heavy derivations in the client, adds additive local SQLite indexes for the current transaction and budget-alert query paths, makes sync batching honor payload size as well as entry count for slower networks, and codifies the current repo-side performance budget through `pnpm qa:performance` plus `docs/15_Performance_Budget.md`. Direct device/network profiling evidence is still pending, but the current Android Hermes bundle in this environment is `2,361,232` bytes
